...
$(document).ready(function(){
	...
	//判断是否登录
//	loginVerify();
	var searchObj = getRequest(location.search),
		searchStr = window.location.search.split('&flag='+searchObj.flag).join('');
		onoffconsole && console.log(searchStr);
	if (searchObj.flag=='1') {
		window.location.search = searchStr + '&hospitalName=测试医院2&applyStatu=1';
	} else if (searchObj.flag=='2') {
		window.location.search = searchStr + '&hospitalName=福州市第一医院';
	} else if (searchObj.flag=='3') {
		window.location.search = searchStr + '&docName=专家2';
	}
...
//窗口变化,对应尺寸变化    
	...
	onoffconsole && console.log(objUrl);
	search = arrUrl.join('&');
    ...
    var currentHtml=decodeURI(location.href).trim();
    if (oldHtml) {
    ...
    function setItem(){
        sessionStorage.setItem('oldHtml',currentHtml);
    }
    //退出时初始化sessionStorage状态
    $(".clearStorage").mousedown(function(){
